...
var index = require('./index-674532d7.js');
...
var QRContext = React__default['default'].createContext(null);
var useQuestionnaireResponse = function () {
    var qrContext = React.useContext(QRContext);
    if (!qrContext) {
...
var QuestionnaireResponse = function (_a) {
    ...
    return (React__default['default'].createElement(QRContext.Provider, { value: [qr, onQRChange] }, children));
};
var QRItemContext = React__default['default'].createContext(null);
var QuestionnaireResponseItem = function (_a) {
    ...
    var onQRItemChange = React.useCallback(function (item) {
        return onQRChange(function (qr) { return (index.__assign(index.__assign({}, qr), { item: index.__spreadArray(index.__spreadArray(index.__spreadArray([], qr.item.slice(0, qrItemIndex)), [
                reduceSetStateAction(qrItem, item)
            ]), qr.item.slice(qrItemIndex + 1)) })); });
    }, [linkId, text, onQRChange]);
    var setAnswer = React.useCallback(function (answer) {
        onQRItemChange(function (qrItem) {
            if (!qrItem) {
                return;
            }
            return index.__assign(index.__assign({}, qrItem), { answer: reduceSetStateAction(qrItem.answer, answer) });
        });
    }, []);
    ...
    return (React__default['default'].createElement(QRItemContext.Provider, { value: [index.__assign(index.__assign({}, qrItem), { linkId: linkId, text: text }), onQRItemChange] }, children));
};

var QuantityContext = React__default['default'].createContext(null);
var useQuantity = function (value) {
    var context = React.useContext(QuantityContext);
    ...
    var onValueChange = React.useCallback(function (value) { return onQuantityChange(function (q) { return (index.__assign(index.__assign({}, q), { value: reduceSetStateAction(q.value, value) })); }); }, [onQuantityChange]);
    var onUnitChange = React.useCallback(function (unit) { return onQuantityChange(function (q) { return (index.__assign(index.__assign({}, q), { unit: reduceSetStateAction(q.unit, unit) })); }); }, [onQuantityChange]);
    return index.__assign(index.__assign({}, quantity), { onValueChange: onValueChange, onUnitChange: onUnitChange });
};
...
var Quantity = function (_a) {
    ...
    if (typeof children == "function") {
        ...
        return (React__default['default'].createElement(QuantityRenderChildren, { quantity: quantity, onQuantityChange: onQuantityChange }, children));
    }
    return (
    // pass quanitty state handlers by context
    React__default['default'].createElement(QuantityContext.Provider, { value: [quantity, onQuantityChange] }, children));
};
...
var reactIs_development$1 = index.createCommonjsModule(function (module, exports) {
...
var reactIs = index.createCommonjsModule(function (module) {
...
var reactIs_development = index.createCommonjsModule(function (module, exports) {
...
function stateReducer(state, actionAndChanges) {
    ...
    switch (type) {
        case useCombobox.stateChangeTypes.InputChange:
            return index.__assign(index.__assign({}, changes), { 
        ...
        case useCombobox.stateChangeTypes.InputBlur:
            return index.__assign(index.__assign({}, changes), (changes.selectedItem && {
...
var ChoiceInput = function (_a) {
    ...
    var _f = React.useState(initOptions), displayOptions = _f[0], setDisplayOptions = _f[1];
    var _g = useCombobox({
        items: initOptions,
    ...
    var SuggestionList = (React__default['default'].createElement("ul", index.__assign({}, getMenuProps(), { className: index.classnames("absolute w-full z-20 mt-1 bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm", { invisible: !isOpen, visible: isOpen }), style: comboPosition }), isOpen &&
        displayOptions.map(function (item, index$1) { return (React__default['default'].createElement("li", index.__assign({ className: index.classnames({
                "text-white bg-blue-600": highlightedIndex === index$1,
                "text-gray-900": highlightedIndex !== index$1,
            }, "cursor-pointer select-none relative py-2 pl-3 pr-9"), key: "" + item + index$1 }, getItemProps({ item: item, index: index$1 })), item.display)); })));
    return (React__default['default'].createElement("div", { className: "relative w-full" },
        React__default['default'].createElement("div", index.__assign({}, getComboboxProps(), { className: "w-full" }),
            React__default['default'].createElement("input", index.__assign({ name: name }, getInputProps({
                onFocus: function () {
                    !isOpen && openMenu();
                }
            }), { placeholder: placeholder, size: Math.max((_d = (_c = value === null || value === void 0 ? void 0 : value.display) === null || _c === void 0 ? void 0 : _c.length) !== null && _d !== void 0 ? _d : 0, (_e = placeholder === null || placeholder === void 0 ? void 0 : placeholder.length) !== null && _e !== void 0 ? _e : 0) + 1, type: "text", className: index.classnames("block w-full", className) })),
            React__default['default'].createElement("button", index.__assign({ type: "button", tabIndex: -1, className: "absolute focus:z-20 inset-y-0 right-0 pr-3 flex items-center pointer-events-none" }, getToggleButtonProps(), { "aria-label": "toggle menu" }),
                React__default['default'].createElement(SelectorIcon, { className: "h-5 w-5 text-gray-400", "aria-hidden": "true" }))),
        containerRef && containerRef.current ? ReactDOM__default['default'].createPortal(SuggestionList, containerRef.current) : SuggestionList));
};
...
var Footer = function (_a) {
    var clear = _a.clear;
    return (React__default['default'].createElement("div", { className: "pt-5" },
        React__default['default'].createElement("div", { className: "flex justify-end" },
            React__default['default'].createElement("button", { type: "button", onClick: clear, className: "bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" }, "Clear"),
            React__default['default'].createElement("button", { type: "submit", className: "ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" }, "Save"))));
};
var Form = function (_a) {
    ...
    return (React__default['default'].createElement("form", { className: "space-y-8 divide-y divide-gray-200", onSubmit: handleSubmit },
        React__default['default'].createElement("div", { className: "space-y-8 divide-y divide-gray-200 sm:space-y-5" },
            React__default['default'].createElement("div", null,
                !hideTitle && (React__default['default'].createElement("div", null,
                    React__default['default'].createElement("h3", { className: "text-lg leading-6 font-medium text-gray-900" }, title),
                    React__default['default'].createElement("p", { className: "mt-1 max-w-2xl text-sm text-gray-500" }))),
                React__default['default'].createElement("div", { className: "mt-6 sm:mt-5 space-y-6 sm:space-y-5" }, React__default['default'].Children.map(children, function (c) { return (React__default['default'].createElement("div", { className: "sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5" }, c)); })))),
        React__default['default'].createElement(Footer, { clear: handleClear })));
};
var BaselineTumorFactors = function (_a) {
    ...
    var _b = React.useState(initQuestionnaireResponse !== null && initQuestionnaireResponse !== void 0 ? initQuestionnaireResponse : initBaselineTumorFactors), qr = _b[0], setQR = _b[1];
    var handleSubmit = function (event) {
        event.preventDefault();
        onSubmit && onSubmit(qr);
    };
    return (React__default['default'].createElement(QuestionnaireResponse, { questionnaireResponse: qr, onQuestionnaireResponseChange: setQR },
        React__default['default'].createElement(Form, { title: "Baseline Tumor Factors", handleSubmit: handleSubmit, handleClear: function () { return setQR(initBaselineTumorFactors); } },
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "DIAGDATE" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "DIAGDATE", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Histological diagnosis date"),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement("div", { className: "mt-1 relative rounded-md w-full shadow-sm " },
                            React__default['default'].createElement("input", { name: "DIAGDATE", type: "date", placeholder: "dd/mm/yyyy", value: answer[0].valueDate, onChange: function (event) { return setAnswer([{ valueDate: event.target.value }]); }, className: "focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 sm:text-sm border-gray-300 rounded-md hide-arrows" })))));
            }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PSA" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "PSA", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "PSA level at histological diagnosis"),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement(Quantity, { quantity: answer[0].valueQuantity, onQuantityChange: function (q) { return setAnswer(function (a) { return [{ valueQuantity: reduceSetStateAction(a[0].valueQuantity, q) }]; }); } }, function (_a) {
                            var value = _a.value, unit = _a.unit, onValueChange = _a.onValueChange;
                            return (React__default['default'].createElement("div", { className: "mt-1 relative rounded-md w-full shadow-sm " },
                                React__default['default'].createElement("input", { name: "PSA", type: "number", min: 0, step: 0.01, value: value, onChange: function (event) { return onValueChange(parseFloat(event.target.value)); }, placeholder: "0.00", className: "focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 sm:text-sm border-gray-300 rounded-md hide-arrows placeholder-gray-300" }),
                                React__default['default'].createElement("div", { className: "absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none" },
                                    React__default['default'].createElement("span", { className: "text-gray-500 sm:text-sm bg-white" }, unit))));
                        }))));
            }),
            React__default['default'].createElement(React__default['default'].Fragment, null,
                React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Clinical stage"),
                React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                    React__default['default'].createElement("div", { className: "mt-1 w-full relative flex border border-gray-300  bg-white rounded-md" },
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "TNMCT" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "TNMCT", className: "sr-only" }, "Clinical tumor stage"),
                                React__default['default'].createElement(ChoiceInput, { name: "TNMCT", options: cTOptions, placeholder: "cT...", className: "focus:ring-blue-500 focus:border-blue-500 relative focus:z-10 block flex-1 sm:text-sm border-transparent rounded-l-md", value: answer[0].valueCoding, onValueChange: function (coding) { return setAnswer([{ valueCoding: coding }]); } })));
                        }),
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "TNMCN" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "TNMCN", className: "sr-only" }, "Clinical nodal stage"),
            ...
            React__default['default'].createElement(React__default['default'].Fragment, null,
                React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Ratio of positive biopsy cores"),
                React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                    React__default['default'].createElement("fieldset", { name: "BIOCORERATIO", className: "mt-1 w-full relative flex border border-gray-300 bg-white rounded-md shadow-sm " },
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "BIOPCORE" }, function (_a) {
                            var _b, _c;
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "BIOPCORE", className: "sr-only" }, "Number of biopsy cores taken"),
                                React__default['default'].createElement("input", { type: "number", value: (_c = (_b = answer[0].valueInteger) === null || _b === void 0 ? void 0 : _b.toString()) !== null && _c !== void 0 ? _c : "", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, min: 0, step: 1, placeholder: "...", size: 4, name: "BIOPCORE", className: "focus:ring-blue-500 focus:border-blue-500 block w-full flex-1 pl-7 sm:text-sm border-transparent rounded-md" })));
                        }),
                        React__default['default'].createElement("div", { className: "mx-3 flex items-center text-lg text-gray-400" },
                            React__default['default'].createElement("span", null, "/")),
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "BIOPPOS" }, function (_a) {
                            var _b, _c;
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "BIOPPOS", className: "sr-only" }, "Number of biopsy cores positive"),
                                React__default['default'].createElement("input", { type: "number", value: (_c = (_b = answer[0].valueInteger) === null || _b === void 0 ? void 0 : _b.toString()) !== null && _c !== void 0 ? _c : "", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, min: 0, step: 1, size: 4, placeholder: "...", name: "BIOPPOS", className: "focus:ring-blue-500 focus:border-blue-500 block w-full flex-1 pl-7 sm:text-sm border-transparent rounded-md" })));
                        })))),
            React__default['default'].createElement(React__default['default'].Fragment, null,
                React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Gleason score"),
                React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                    React__default['default'].createElement("fieldset", { className: "mt-1 relative flex rounded-md w-full shadow-sm border border-gray-300 bg-white" },
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "GLEASONBIOP1" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "GLEASONBIOP1", className: "sr-only" }, "Primary Gleason score"),
                                React__default['default'].createElement("input", { type: "number", step: 1, min: 1, max: 5, value: answer[0].valueInteger, onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, name: "GLEASONBIOP1", className: "focus:ring-blue-500 focus:border-blue-500 focus:z-10 block w-full flex-1 pl-7 sm:text-sm border-transparent rounded-md" })));
                        }),
                        React__default['default'].createElement("div", { className: "mx-3 flex items-center text-lg text-gray-400" },
                            React__default['default'].createElement("span", null, "+")),
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "GLEASONBIOP2" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "GLEASONBIOP2", className: "sr-only" }, "Secondary Gleason score"),
...
var PathologyInformation = function (_a) {
    ...
    var _b = React.useState(initQuestionnaireResponse !== null && initQuestionnaireResponse !== void 0 ? initQuestionnaireResponse : initPathologicalInformation), qr = _b[0], setQR = _b[1];
    var pNOptions = [
    ...
    return (React__default['default'].createElement(QuestionnaireResponse, { questionnaireResponse: qr, onQuestionnaireResponseChange: setQR },
        React__default['default'].createElement(Form, { title: "Pathological information", handleSubmit: handleSubmit, handleClear: function () { return setQR(initPathologicalInformation()); } },
            React__default['default'].createElement(React__default['default'].Fragment, null,
                React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Clinical stage"),
                React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                    React__default['default'].createElement("div", { className: "mt-1 w-full relative flex border border-gray-300  bg-white rounded-md" },
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "TNMPT" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "TNMPT", className: "sr-only" }, "Clinical tumor stage"),
                        ...
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "TNMPN" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "TNMPN", className: "sr-only" }, "Clinical nodal stage"),
            ...
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "MARGIN" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Margin status"),
                    React__default['default'].createElement("fieldset", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement("div", { className: "flex items-center" },
                            React__default['default'].createElement("input", { id: "marging-positive", type: "radio", name: "MARGIN", checked: answer[0].valueInteger === 1, value: "1", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, className: "focus:ring-blue-500 text-blue-600 border-gray-300 h-4 w-4 rounded-md" }),
                            React__default['default'].createElement("label", { htmlFor: "margin-positive", className: "ml-3 block text-sm font-medium text-gray-700" }, "positive")),
                        React__default['default'].createElement("div", { className: "flex items-center" },
                            React__default['default'].createElement("input", { id: "margin-negative", type: "radio", name: "MARGIN", checked: answer[0].valueInteger === 0, value: "0", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, className: "focus:ring-blue-500 text-blue-600 border-gray-300 h-4 w-4 rounded-md" }),
                            React__default['default'].createElement("label", { htmlFor: "margin-negative", className: "ml-3 block text-sm font-medium text-gray-700" }, "negative")))));
            }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "MARGINFOC" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Margin status focality"),
                    React__default['default'].createElement("fieldset", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement("div", { className: "flex items-center" },
                            React__default['default'].createElement("input", { id: "marging-focality-focal", type: "radio", name: "MARGINFOC", checked: answer[0].valueInteger === 1, value: "1", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, className: "focus:ring-blue-500 text-blue-600 border-gray-300 h-4 w-4 rounded-md" }),
                            React__default['default'].createElement("label", { htmlFor: "margin-focality-focal", className: "ml-3 block text-sm font-medium text-gray-700" }, "focal")),
                        React__default['default'].createElement("div", { className: "flex items-center" },
                            React__default['default'].createElement("input", { id: "margin-focality-focal", type: "radio", name: "MARGINFOC", checked: answer[0].valueInteger === 2, value: "2", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, className: "focus:ring-blue-500 text-blue-600 border-gray-300 h-4 w-4 rounded-md" }),
                            React__default['default'].createElement("label", { htmlFor: "margin-focality-multi", className: "ml-3 block text-sm font-medium text-gray-700" }, "multi-focal")))));
            }),
            React__default['default'].createElement(React__default['default'].Fragment, null,
                React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Gleason score"),
                React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                    React__default['default'].createElement("fieldset", { className: "mt-1 relative flex rounded-md w-full shadow-sm border border-gray-300 bg-white" },
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "GLEASONPATH1" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "GLEASONPATH1", className: "sr-only" }, "Primary Gleason score"),
                        ...
                        React__default['default'].createElement("div", { className: "mx-3 flex items-center text-lg text-gray-400" },
                            React__default['default'].createElement("span", null, "+")),
                        React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "GLEASONPATH2" }, function (_a) {
                            var answer = _a.answer, setAnswer = _a.setAnswer;
                            return (React__default['default'].createElement(React__default['default'].Fragment, null,
                                React__default['default'].createElement("label", { htmlFor: "GLEASONPATH2", className: "sr-only" }, "Secondary Gleason score"),
...
var QuestionStartStopOngoing = function (_a) {
    ...
    return (React__default['default'].createElement(React__default['default'].Fragment, null,
        React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, label),
        React__default['default'].createElement("fieldset", null,
            React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                React__default['default'].createElement("div", { className: "mt-1 relative rounded-md w-full flex shadow-sm border border-gray-300 bg-white" },
                    React__default['default'].createElement(QuestionnaireResponseItem, { linkId: startVariableID }, function (_a) {
                        var answer = _a.answer, setAnswer = _a.setAnswer;
                        return (React__default['default'].createElement("input", { name: startVariableID, type: "date", placeholder: "dd/mm/yyyy", value: answer[0].valueDate, onChange: function (event) { return setAnswer([{ valueDate: event.target.value }]); }, className: "focus:ring-blue-500 focus:border-blue-500 block w-0.5 flex-1 sm:text-sm border-transparent rounded-md" }));
                    }),
                    React__default['default'].createElement("div", { className: "mx-3 flex items-center text-lg text-gray-400" },
                        React__default['default'].createElement("span", null, "\u2192")),
                    React__default['default'].createElement(QuestionnaireResponseItem, { linkId: stopVariableId }, function (_a) {
                        var answer = _a.answer, setAnswer = _a.setAnswer;
                        return (React__default['default'].createElement("input", { name: stopVariableId, type: "date", placeholder: "dd/mm/yyyy", value: answer[0].valueDate, onChange: function (event) { return setAnswer([{ valueDate: event.target.value }]); }, className: "focus:ring-blue-500 focus:border-blue-500 block w-0.5 flex-1 sm:text-sm border-transparent rounded-md" }));
                    })),
                React__default['default'].createElement(QuestionnaireResponseItem, { linkId: ongoingVariableId }, function (_a) {
                    var answer = _a.answer, setAnswer = _a.setAnswer;
                    return (React__default['default'].createElement("div", { className: "mt-2 relative w-full flex" },
                        React__default['default'].createElement("input", { type: "checkbox", name: ongoingVariableId, checked: answer[0].valueInteger === 0, value: "0", onChange: function (event) { return setAnswer([{ valueInteger: !!answer[0].valueInteger ? 1 - answer[0].valueInteger : 1 }]); }, className: "focus:ring-blue-500 text-blue-600 border-gray-300 h-4 w-4 rounded-md" }),
                        React__default['default'].createElement("label", { htmlFor: "margin-negative", className: "ml-3 block text-sm font-medium text-gray-700" }, "ongoing")));
                })))));
};
var QuestionDate = function (_a) {
    var label = _a.label, variableID = _a.variableID;
    return (React__default['default'].createElement(QuestionnaireResponseItem, { linkId: variableID }, function (_a) {
        var answer = _a.answer, setAnswer = _a.setAnswer;
        return (React__default['default'].createElement(React__default['default'].Fragment, null,
            React__default['default'].createElement("label", { htmlFor: variableID, className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, label),
            React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                React__default['default'].createElement("input", { name: variableID, type: "date", placeholder: "dd/mm/yyyy", value: answer[0].valueDate, onChange: function (event) { return setAnswer([{ valueDate: event.target.value }]); }, className: "focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 sm:text-sm border-gray-300 rounded-md shadow-sm" }))));
    }));
};
var TreatmentVariables = function (_a) {
    _a.author; _a.subject; var onSubmit = _a.onSubmit; _a.hideTitle; var initQuestionnaireResponse = _a.initQuestionnaireResponse;
    var _b = React.useState(initQuestionnaireResponse !== null && initQuestionnaireResponse !== void 0 ? initQuestionnaireResponse : initTreatmentVariables), qr = _b[0], setQR = _b[1];
    ...
    return (React__default['default'].createElement(QuestionnaireResponse, { questionnaireResponse: qr, onQuestionnaireResponseChange: setQR },
        React__default['default'].createElement(Form, { title: "Threatment variables", handleSubmit: handleSubmit, handleClear: function () { return setQR(initTreatmentVariables()); } },
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PRIMARYTX" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "PRIMARYTX", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Treatment modalities used"),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement(ChoiceInput, { value: answer[0].valueCoding, onValueChange: function (q) { return setAnswer(function (a) { return [{ valueCoding: answer[0].valueCoding }]; }); }, name: "PRIMARYTX", options: primaryTherapyOptions, placeholder: "...", className: "focus:ring-blue-500 focus:border-blue-500 block w-full flex-grow pl-7 sm:text-sm border-gray-300 rounded-md shadow-sm placeholder-gray-300" }))));
            }),
            React__default['default'].createElement(QuestionDate, { variableID: "PRWATCHDATE", label: "Date watchful waiting initiated" }),
            React__default['default'].createElement(QuestionDate, { variableID: "PRACTIVEDATE", label: "Date active surveillance initiated" }),
            React__default['default'].createElement(QuestionDate, { variableID: "PRRADPROTXDATE", label: "Date of primary pradical prostatectomy" }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PRNERVESPARE" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("legend", { className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Primary nerve sparing status"),
                    React__default['default'].createElement("fieldset", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement("div", { className: "flex items-center" },
                            React__default['default'].createElement("input", { id: "PRNERVESPARE-non-nerve-sparing", type: "radio", name: "PRNERVESPARE", checked: answer[0].valueInteger === 1, value: "1", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, className: "focus:ring-blue-500 text-blue-600 border-gray-300 h-4 w-4 rounded-md" }),
                            React__default['default'].createElement("label", { htmlFor: "margin-focality-focal", className: "ml-3 block text-sm font-medium text-gray-700" }, "non nerve-sparing")),
                        React__default['default'].createElement("div", { className: "flex items-center" },
                            React__default['default'].createElement("input", { id: "PRNERVESPARE-nerve-sparing", type: "radio", name: "PRNERVESPARE", checked: answer[0].valueInteger === 2, value: "2", onChange: function (event) { return setAnswer([{ valueInteger: parseInt(event.target.value) }]); }, className: "focus:ring-blue-500 text-blue-600 border-gray-300 h-4 w-4 rounded-md" }),
                            React__default['default'].createElement("label", { htmlFor: "margin-focality-multi", className: "ml-3 block text-sm font-medium text-gray-700" }, "nerve-sparing")))));
            }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PREBRTTOTDOSE" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "PREBRTTOTDOSE", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Primary EBRT dose"),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement(Quantity, { quantity: answer[0].valueQuantity, onQuantityChange: function (q) { return setAnswer(function (a) { return [{ valueQuantity: reduceSetStateAction(a[0].valueQuantity, q) }]; }); } }, function (_a) {
                            var value = _a.value, unit = _a.unit, onValueChange = _a.onValueChange;
                            return (React__default['default'].createElement("div", { className: "mt-1 relative rounded-md w-full shadow-sm " },
                                React__default['default'].createElement("input", { name: "PREBRTTOTDOSE", type: "number", min: 0, step: 0.01, value: value, onChange: function (event) { return onValueChange(parseFloat(event.target.value)); }, placeholder: "0.00", className: "focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 sm:text-sm border-gray-300 rounded-md hide-arrows placeholder-gray-300" }),
                                React__default['default'].createElement("div", { className: "absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none" },
                                    React__default['default'].createElement("span", { className: "text-gray-500 sm:text-sm bg-white" }, unit))));
                        }))));
            }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PREBRTDOSEPERFRACT" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "PREBRTDOSEPERFRACT", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Primary EBRT average dose per fraction"),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement(Quantity, { quantity: answer[0].valueQuantity, onQuantityChange: function (q) { return setAnswer(function (a) { return [{ valueQuantity: reduceSetStateAction(a[0].valueQuantity, q) }]; }); } }, function (_a) {
                            var value = _a.value, unit = _a.unit, onValueChange = _a.onValueChange;
                            return (React__default['default'].createElement("div", { className: "mt-1 relative rounded-md w-full shadow-sm " },
                                React__default['default'].createElement("input", { name: "PREBRTDOSEPERFRACT", type: "number", min: 0, step: 0.01, value: value, onChange: function (event) { return onValueChange(parseFloat(event.target.value)); }, placeholder: "0.00", className: "focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 sm:text-sm border-gray-300 rounded-md hide-arrows placeholder-gray-300" }),
                                React__default['default'].createElement("div", { className: "absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none" },
                                    React__default['default'].createElement("span", { className: "text-gray-500 sm:text-sm bg-white" }, unit))));
                        }))));
            }),
            React__default['default'].createElement(QuestionStartStopOngoing, { startVariableID: "PREBRTTXSTARTDATE", stopVariableId: "PREBRTTXSTOPDATE", ongoingVariableId: "PREBRTTXONGOING", label: "Period of primary EBRT" }),
            React__default['default'].createElement(QuestionStartStopOngoing, { startVariableID: "PRBRACHYTXSTARTDATE", stopVariableId: "PRBRACHYTXSTOPDATE", ongoingVariableId: "PRBRACHYTXONGOING", label: "Period of primary brachytherapy" }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PRBRACHYDOSERATE" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "PRBRACHYDOSERATE", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Primary branchytherapy dose:"),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement(Quantity, { quantity: answer[0].valueQuantity, onQuantityChange: function (q) { return setAnswer(function (a) { return [{ valueQuantity: reduceSetStateAction(a[0].valueQuantity, q) }]; }); } }, function (_a) {
                            var value = _a.value, unit = _a.unit, onValueChange = _a.onValueChange;
                            return (React__default['default'].createElement("div", { className: "mt-1 relative rounded-md w-full shadow-sm " },
                                React__default['default'].createElement("input", { name: "PRBRACHYDOSERATE", type: "number", min: 0, step: 0.01, value: value, onChange: function (event) { return onValueChange(parseFloat(event.target.value)); }, placeholder: "0.00", className: "focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 sm:text-sm border-gray-300 rounded-md hide-arrows placeholder-gray-300" }),
                                React__default['default'].createElement("div", { className: "absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none" },
                                    React__default['default'].createElement("span", { className: "text-gray-500 sm:text-sm bg-white" }, unit))));
                        }))));
            }),
            React__default['default'].createElement(QuestionStartStopOngoing, { startVariableID: "PRADTTXSTARTDATE", stopVariableId: "PRADTTXSTOPDATE", ongoingVariableId: "PRADTTXONGOING", label: "Period of primary ADT" }),
            React__default['default'].createElement(QuestionStartStopOngoing, { startVariableID: "PRBRACHYTXSTARTDATE", stopVariableId: "PRBRACHYTXSTOPDATE", ongoingVariableId: "PRBRACHYTXONGOING", label: "Period of primary brachytherapy" }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PRIMARYTXFT" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "PRIMARYTXFT", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Indicate the type of focal therapy used for this patient"),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement("textarea", { className: "focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md placeholder-gray-300", value: answer[0].valueString, onChange: function (event) { return setAnswer([{ valueString: event.target.value }]); } }))));
            }),
            React__default['default'].createElement(QuestionStartStopOngoing, { startVariableID: "PRFOCTXSTARTDATE", stopVariableId: "PRFOCTXSTOPDATE", ongoingVariableId: "PRFOCTXONGOING", label: "Period of primary focal therapy" }),
            React__default['default'].createElement(QuestionnaireResponseItem, { linkId: "PRIMARYTXOTHER" }, function (_a) {
                var answer = _a.answer, setAnswer = _a.setAnswer;
                return (React__default['default'].createElement(React__default['default'].Fragment, null,
                    React__default['default'].createElement("label", { htmlFor: "PRIMARYTXOTHER", className: "block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" }, "Indicate the other primary treatment modality used                               "),
                    React__default['default'].createElement("div", { className: "mt-1 sm:mt-0 sm:col-span-2 md:col-span-1" },
                        React__default['default'].createElement("textarea", { className: "focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md placeholder-gray-300", value: answer[0].valueString, onChange: function (event) { return setAnswer([{ valueString: event.target.value }]); } }))));
            }),
            React__default['default'].createElement(QuestionStartStopOngoing, { startVariableID: "PROTHERTXSTARTDATE", stopVariableId: "PROTHERTXSTOPDATE", ongoingVariableId: "PROTHERTXONGOING", label: "Period of primary other therapy" }))));
};
...
var DynamicQuestionnaire = function (_a) {
    var questionnaire = _a.questionnaire, props = index.__rest(_a, ["questionnaire"]);
    switch (questionnaire) {
        case "http://tiro.health/fhir/Questionnaire/ichom-lpc-baseline-tumor-factors|0.1":
            return React__default['default'].createElement(BaselineTumorFactors, index.__assign({}, props));
        case "http://tiro.health/fhir/Questionnaire/ichom-lpc-pathology-info|0.1":
            return React__default['default'].createElement(PathologyInformation, index.__assign({}, props));
        case "http://tiro.health/fhir/Questionnaire/ichom-lpc-treatment-variables|0.1":
            return React__default['default'].createElement(TreatmentVariables, index.__assign({}, props));
    }
};

exports.BaselineTumorFactors = BaselineTumorFactors;
exports.DynamicQuestionnaire = DynamicQuestionnaire;
exports.PathologyInformation = PathologyInformation;
exports.TreatmentVariables = TreatmentVariables;
exports.getQuestionnaire = getQuestionnaire;
exports.getQuestionnaireIDs = getQuestionnaireIDs;
//# sourceMappingURL=Questionnaire-c40f74ce.js.map
